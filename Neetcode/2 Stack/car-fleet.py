...
class Solution:
  def carFleet(self, target: int, position: list[int], speed: list[int]) -> int:
    ...
    position = [2,4,6]
    speed = [4,2,1]
    # Sort the cars by position in descending order. We want to start at the 
    #   car that is closest to the target. 
    pairs = []
    stack = []
    for car in range(len(position)):
      pairs.append([position[car], speed[car]])
    
    # Sort descending
    pairs = sorted(pairs)[::-1]
    print(pairs)
    # Go through the pairs and cacluate arrival times
    for pos, sp in pairs:
